...
typedef struct {
 ...
 typedef struct {
	 uint16_t volt_set_V;      // Điện áp yêu cầu sạc (0.1V/bit)
	 uint16_t curr_set_A;      // Dòng yêu cầu sạc (0.1A/bit)
	 uint8_t chg_sw;           // Trạng thái sạc: 0 = bật, 1 = tắt
	 uint8_t mode;             // 0 = chế độ sạc, 1 = chế độ sưởi
 } BMS_ChgInfo_t;
